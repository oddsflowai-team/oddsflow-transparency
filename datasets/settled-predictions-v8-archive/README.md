# OddsFlow Settled Predictions — V8 Engine Archive (frozen)

> **This dataset is closed. It is kept for the record, not for reuse.**
>
> | | |
> |---|---|
> | Engine | V8 (retired) |
> | Coverage | 2026-01-28 → 2026-08-24 |
> | Rows | 2,473 settled bets across 1,629 matches, 98 competitions |
> | Last write | 2026-08-24T04:15:50Z |
> | Status | **Frozen. Nothing will be added.** |
>
> **For the live record, use [`../settled-predictions/`](../settled-predictions/).**
>
> The V8 engine stopped writing on 2026-08-24. OddsFlow's signal engine did not
> stop — it moved to V9, and the live dataset is generated from V9's per-bet log.
> These files are **not** re-exported, re-stated or corrected: they are exactly
> what was published while V8 was running. Rewriting a published prediction after
> the fact is the one thing a track record cannot survive, so we did not.
>
> **`pressure_signal` exists only here.** V9 does not record that column, so the
> live dataset has no equivalent. If you want the model's momentum read at the
> moment of entry, this is the only place it is published.
>
> Two other differences from the live dataset, so a comparison is not misread:
>
> - **Row grain.** Here one row is roughly one decision. In the live dataset one
>   row is one *model × source*, so the same decision taken by two engines or
>   placed at two brokers appears more than once. Row counts between the two are
>   therefore not comparable; `model_key` and `source` are published there so you
>   can collapse them yourself.
> - **Markets.** This archive holds Asian Handicap and Over/Under only. The live
>   dataset also carries 1X2.
...
<!--SNAPSHOT:START-->
**Settled-predictions dataset (snapshot: 2026-08-26):**
